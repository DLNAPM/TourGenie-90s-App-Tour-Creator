...
export class TourService {
  ...
  async generateSceneVideo(scene: Scene, screenshot?: string): Promise<string> {
    ...
    const cleanPrompt = scene.visualPrompt ? `${scene.visualPrompt.trim()}. ${englishVisualGuard}` : `Cinematic clean mobile and desktop app interface interaction in motion. ${englishVisualGuard}`;
