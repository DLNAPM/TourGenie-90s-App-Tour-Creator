...
export default function App() {
    ...
    if (editorState.clips && editorState.clips.length > 0) {
      clipsToSave = editorState.clips.map((c, idx) => {
        ...
        return {
          ...
          useVoiceover: c.useVoiceover !== false,
          narrationStartOffset: Math.max(0, c.narrationStartOffset ?? matchingScene?.narrationStartOffset ?? 0)
        };
      });
      ...
      clipsToSave = state.scenes.map((s, idx) => {
        ...
        return {
          ...
          narrationStartOffset: Math.max(0, s.narrationStartOffset || 0)
        };
      });
    ...
    if (session.clips && session.clips.length > 0) {
      restoredClips = session.clips.map((c: any, index: number) => {
        ...
        return {
          ...
          audioUrl: c?.audioUrl || '',
          useVoiceover: c?.useVoiceover !== false,
          narrationStartOffset: Math.max(0, Number(c?.narrationStartOffset) || 0)
        };
      });
    ...
    for (let i = 0; i < state.scenes.length; i++) {
      ...
      try {
        ...
        newClips.push({
          ...
          useVoiceover: scene.useVoiceover !== false,
          narrationStartOffset: Math.max(0, scene.narrationStartOffset || 0)
        });
      } catch (err) {
  ...
  const handleConfirmMatchAndStitch = async (matchedItems: SceneMatchItem[]) => {
    const updatedClips: EditorClip[] = matchedItems.map((item, idx) => ({
      ...
      duration: Math.max(30, item.duration || 30),
      status: 'ready',
      ...
      useVoiceover: item.useVoiceover !== false,
      narrationStartOffset: Math.max(0, item.narrationStartOffset || 0)
    }));
  ...
  const scenesForMatcher: SceneMatchItem[] = useMemo(() => {
    if (editorState.clips.length > 0) {
      return editorState.clips.map((clip, idx) => ({
        ...
        useVoiceover: clip.useVoiceover !== false,
        narrationStartOffset: Math.max(0, clip.narrationStartOffset || 0)
      }));
    }
  ...
  const reRenderScene = async (sceneIndex: number, motionStyle: any) => {
    ...
    try {
      const videoUrl = await tourService.generateSceneVideo(scene, screenshot, {
        ...
        narrationStartOffset: scene.narrationStartOffset || 0
      });
  ...
  const handleRenderProject = async (options?: { autoOpenPreview?: boolean; customClips?: EditorClip[] }): Promise<string | null> => {
    ...
    try {
      ...
      try {
        ...
        // Attach matching synchronized audio track for each scene if narrator voice is chosen
        const voiceoverMap: Record<string, boolean> = {};
        const durationsMap: number[] = [];
        const startOffsetsMap: Record<string, number> = {};

        clipsToRender.forEach((clip, idx) => {
          const shouldUseVoice = clip.useVoiceover !== false && editorState.includeVoiceover !== false;
          voiceoverMap[`scene-${idx + 1}`] = shouldUseVoice;
          voiceoverMap[`${idx + 1}`] = shouldUseVoice;
          voiceoverMap[idx.toString()] = shouldUseVoice;
          durationsMap.push(Math.max(30, clip.duration || 30));
          const offset = Math.max(0, clip.narrationStartOffset || 0);
          startOffsetsMap[`scene-${idx + 1}`] = offset;
          startOffsetsMap[`${idx + 1}`] = offset;
          startOffsetsMap[idx.toString()] = offset;

          if (shouldUseVoice && clip.audioUrl) {
            try {
        ...
        formData.append('voiceoverConfig', JSON.stringify(voiceoverMap));
        formData.append('durations', JSON.stringify(durationsMap));
        formData.append('startOffsets', JSON.stringify(startOffsetsMap));
        ...
        const voiceoverFlags = clipsToRender.map(c => c.useVoiceover !== false && editorState.includeVoiceover !== false);
      ...
      for (let i = 0; i < updatedScenes.length; i++) {
        ...
        try {
          ...
          const videoUrl = await tourService.generateSceneVideo(updatedScenes[i], screenshot, {
            ...
            narrationStartOffset: updatedScenes[i].narrationStartOffset || 0
          });
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/80 pb-6">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl text-sm transition flex items-center gap-2 shadow-lg shadow-indigo-100 active:scale-95"
                    >
                      <ArrowsRightLeftIcon className="w-4 h-4 text-white" /> Choose Voices & Stitch Master ({state.scenes.length})
                    </button>
                    <button
                      onClick={sendScenesToEditor}
                      className="bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 text-indigo-700 font-bold py-3 px-5 rounded-xl shadow-sm flex items-center gap-2 text-sm transition active:scale-95"
                    >
                      <FilmIcon className="w-4 h-4 text-indigo-600" /> Open in Video Editor ({state.scenes.length} Scenes)
                    </button>
                    <button
                      onClick={async () => {
                        await sendScenesToEditor();
                    ...
                    <div key={scene.id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setState(prev => ({
                                    ...prev,
                                    scenes: prev.scenes.map((s, i) => i === idx ? { ...s, useVoiceover: s.useVoiceover === false ? true : false } : s)
                                  }));
                                }}
                                className={`px-2 py-0.5 rounded-md text-[11px] font-bold flex items-center gap-1 transition border ${
                                  scene.useVoiceover !== false
                                    ? 'bg-emerald-50 border-emerald-300 text-emerald-700 hover:bg-emerald-100'
                                    : 'bg-slate-100 border-slate-300 text-slate-500 hover:bg-slate-200'
                                }`}
                                title={scene.useVoiceover !== false ? "Narrator voice is ACTIVE. Click to mute." : "Narrator voice is MUTED. Click to enable."}
                              >
                                {scene.useVoiceover !== false ? (
                                  <>
                                    <SpeakerWaveIcon className="w-3 h-3 text-emerald-600" />
                                    <span>Voice: On</span>
                                  </>
                                ) : (
                                  <>
                                    <SpeakerXMarkIcon className="w-3 h-3 text-slate-400" />
                          ...
                          <p className="text-slate-700 text-sm mb-3 leading-relaxed font-medium">"{scene.narration}"</p>

                          {/* Speech Start Timing Control */}
                          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-2.5 space-y-1.5">
                            <div className="flex items-center justify-between text-[11px]">
                              ...
                              <span className="font-mono font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-1.5 py-0.5 rounded text-[10px]">
                                {(scene.narrationStartOffset || 0) === 0 ? '0.0s (Immediately)' : `+${(scene.narrationStartOffset || 0).toFixed(1)}s into scene`}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              <input
                                type="range"
                                min="0"
                                max={Math.min(25, Math.floor((scene.duration || 30) - 5))}
                                step="0.5"
                                value={scene.narrationStartOffset || 0}
                                onChange={(e) => {
                                  const val = parseFloat(e.target.value);
                                  setState(prev => ({
                                    ...prev,
                                    scenes: prev.scenes.map((s, i) => i === idx ? { ...s, narrationStartOffset: val } : s)
                                  }));
                                }}
                                className="flex-1 h-1.5 bg-slate-200 rounded-lg cursor-pointer accent-indigo-600"
                              />
                                ...
                                {[0, 1, 2, 3].map(sec => (
                                  <button
                                    ...
                                    onClick={() => {
                                      setState(prev => ({
                                        ...prev,
                                        scenes: prev.scenes.map((s, i) => i === idx ? { ...s, narrationStartOffset: sec } : s)
                                      }));
                                    }}
                                    className={`px-1.5 py-0.5 rounded text-[10px] font-semibold border transition ${
                                      (scene.narrationStartOffset || 0) === sec
                        ...
                        <div key={clip.id} className="group relative bg-slate-50 border border-slate-200/80 rounded-2xl p-4 flex flex-col sm:flex-row gap-5 items-start transition hover:border-indigo-300 hover:bg-white hover:shadow-md">
                          ...
                          <div className="flex-1 w-full space-y-3 pt-1">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setEditorState(prev => {
                                      const next = [...prev.clips];
                                      next[idx] = { ...next[idx], useVoiceover: next[idx].useVoiceover === false ? true : false };
                                      return { ...prev, clips: next, isRendered: false };
                                    });
                                  }}
                                  className={`px-2 py-0.5 rounded-md text-[11px] font-bold flex items-center gap-1 transition border ${
                                    clip.useVoiceover !== false
                                      ? 'bg-emerald-50 border-emerald-300 text-emerald-700 hover:bg-emerald-100'
                                      : 'bg-slate-100 border-slate-300 text-slate-500 hover:bg-slate-200'
                                  }`}
                                  title={clip.useVoiceover !== false ? "Narrator voice is ACTIVE. Click to mute." : "Narrator voice is MUTED. Click to enable."}
                                >
                                  {clip.useVoiceover !== false ? (
                                    <>
                                      <SpeakerWaveIcon className="w-3 h-3 text-emerald-600" />
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 space-y-2 shadow-sm">
                              ...
                              {/* Speech Start Timing Control */}
                              <div className="bg-slate-50 border border-slate-200/80 rounded-lg p-2 flex flex-col sm:flex-row sm:items-center justify-between gap-2 mt-1">
                                <div className="flex items-center gap-1.5">
                                  <ClockIcon className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0" />
                                  <span className="text-[11px] font-bold text-slate-700">Speech Starts:</span>
                                  <span className="font-mono font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-1.5 py-0.2 rounded text-[10px]">
                                    {(clip.narrationStartOffset || 0) === 0 ? '0.0s (Immediate)' : `+${(clip.narrationStartOffset || 0).toFixed(1)}s into scene`}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2 flex-1 max-w-xs">
                                  <input
                                    type="range"
                                    min="0"
                                    max={Math.min(25, Math.floor((clip.duration || 30) - 5))}
                                    step="0.5"
                                    value={clip.narrationStartOffset || 0}
                                    onChange={(e) => {
                                      const val = parseFloat(e.target.value);
                                      setEditorState(prev => {
                                        const next = [...prev.clips];
                                        next[idx] = { ...next[idx], narrationStartOffset: val };
                                        return { ...prev, clips: next, isRendered: false };
                                      });
                                    }}
                                    className="flex-1 h-1.5 bg-slate-200 rounded-lg cursor-pointer accent-indigo-600"
                                    ...
                                    {[0, 1, 2, 3].map(sec => (
                                      <button
                                        ...
                                        onClick={() => {
                                          setEditorState(prev => {
                                            const next = [...prev.clips];
                                            next[idx] = { ...next[idx], narrationStartOffset: sec };
                                            return { ...prev, clips: next, isRendered: false };
                                          });
                                        }}
                                        className={`px-1.5 py-0.5 rounded text-[10px] font-semibold border transition ${
                                          (clip.narrationStartOffset || 0) === sec
                                            ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm'
                                            : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'
                                        }`}
                                      >
                        ...
                        {isDurationValid && !editorState.isRendered && (
                          <div className="space-y-2.5">
                            ...
                            >
                              ...
                              Choose Voices & Match Scripts Before Stitching
                            </button>
                            <button
